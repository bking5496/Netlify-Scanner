            ...
            _executeRender() {
                            ...
                            <div class="max-w-4xl mx-auto p-4 sm:p-6">
                                ...
                                <div class="flex gap-3 mb-6">
                                    ...
                                    <button onclick="app.generateQRCodes()" class="flex-1 bg-purple-600 hover:bg-purple-700 text-white py-3 px-4 rounded-xl font-bold transition-colors ${selectedSet.size === 0 ? 'opacity-50' : ''}">
                                        🖨️ Print (${selectedSet.size})
                                    </button>
                                </div>
                                
                                ${locations.length === 0 ? `
                                    <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 p-8 text-center">
                                        <div class="text-4xl mb-4">📦</div>
                                        <h3 class="text-lg font-bold text-slate-700">No Locations Configured</h3>
                                        <p class="text-slate-500 mt-2">Add racks and floor locations in Settings first.</p>
                                        <button onclick="app.hideQRGenerator(); app.showSettings();" class="mt-4 bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-xl font-bold transition-colors">
                                            Go to Settings
                                        </button>
                                    </div>
                                ` : `
                                    ...
                                    <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 overflow-hidden mb-4">
                                        <div class="bg-blue-50 px-4 py-3 border-b border-blue-100">
                                            <h2 class="font-bold text-blue-800">🏭 Rack Locations</h2>
                                        </div>
                                        <div class="divide-y divide-slate-100 max-h-64 overflow-y-auto">
                                            ${locations.filter(l => l.type === 'rack').map(loc => `
                                                <label class="p-4 flex items-center gap-3 hover:bg-slate-50 cursor-pointer">
                                                    ...
                                                    />
                                                    <div class="flex-1">
                                                        <div class="font-bold text-slate-900">${loc.code}</div>
                                                        <div class="text-xs text-slate-500">${loc.label}</div>
                                                    </div>
                                                </label>
                                            `).join('')}
                                        </div>
                                    ...
                                    <div class="bg-white rounded-2xl shadow-lg shadow-slate-200 overflow-hidden">
                                        <div class="bg-teal-50 px-4 py-3 border-b border-teal-100">
                                            <h2 class="font-bold text-teal-800">📦 Floor Locations</h2>
                                        </div>
                                        <div class="divide-y divide-slate-100 max-h-64 overflow-y-auto">
                ...
                if (this.showingSessionDashboard) {
                    ...
                    const renderSessionCard = (session) => {
                        ...
                        return `
                            <div class="bg-white rounded-xl shadow-md p-4 border border-slate-100">
                                <div class="flex items-start justify-between mb-3">
                                    <div>
                                        <div class="flex items-center gap-2 mb-1">
                                            <span class="inline-block px-2 py-0.5 rounded text-xs font-bold bg-${typeColor}-100 text-${typeColor}-700">${session.session_type}</span>
                                            <span class="inline-block px-2 py-0.5 rounded text-xs font-bold bg-${statusColor}-100 text-${statusColor}-700 capitalize">${session.status}</span>
                                        </div>
                                        <div class="font-bold text-slate-900">Session #${session.session_number}</div>
                                        <div class="text-xs text-slate-500">${session.take_date}</div>
                                    </div>
                                    <div class="text-right">
                                        <div class="text-2xl font-bold text-slate-900">${scanCount}</div>
                                        <div class="text-xs text-slate-500">scans</div>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center justify-between pt-3 border-t border-slate-100">
                                    <div class="flex items-center gap-2">
                                        <div class="flex items-center gap-1.5">
                                            <div class="w-2 h-2 rounded-full bg-green-500 ${deviceCount > 0 ? 'animate-pulse' : ''}"></div>
                                            <span class="text-sm font-medium text-slate-600">${deviceCount} active</span>
                                        </div>
                                        <span class="text-slate-300">/</span>
                                        <span class="text-sm text-slate-400">${totalDevices} total</span>
                                    </div>
                    ...
                    updateView(`
                        <div class="min-h-screen bg-slate-50">
                            <div class="bg-indigo-600 text-white p-4 sm:p-6">
                                <div class="max-w-4xl mx-auto">
                                    <button onclick="app.hideSessionDashboard()" class="flex items-center gap-2 text-indigo-100 hover:text-white mb-4 transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                        </svg>
                                        <span class="font-medium">Back</span>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <div>
                                            <h1 class="text-2xl font-bold">📊 Session Dashboard</h1>
                                            <p class="text-indigo-100 text-sm mt-1">Real-time overview of active sessions</p>
                                        </div>
                                        <button onclick="app.refreshDashboard()" class="bg-white/20 hover:bg-white/30 p-2 rounded-lg transition-colors" title="Refresh">
                                            <svg class="w-5 h-5 ${this.dashboardLoading ? 'animate-spin' : ''}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="max-w-4xl mx-auto p-4 sm:p-6">
                                ${this.dashboardLoading ? `
                                    <div class="text-center py-12">
                                        ...
                                        <div class="mt-4 text-slate-500">Loading sessions...</div>
                                    </div>
                                ` : sessions.length === 0 ? `
                                    <div class="text-center py-12">
                                        <div class="text-4xl mb-4">📭</div>
                                        <h3 class="text-lg font-bold text-slate-700">No Active Sessions</h3>
                                        <p class="text-slate-500 mt-1">There are no active or paused sessions at the moment.</p>
                                    </div>
                                ` : `
                                    <!-- Summary Stats -->
                                    <div class="grid grid-cols-3 gap-4 mb-6">
                                        <div class="bg-white rounded-xl p-4 text-center shadow-sm">
                                            <div class="text-3xl font-bold text-indigo-600">${sessions.length}</div>
                                            <div class="text-xs text-slate-500">Total Sessions</div>
                                        </div>
                                        <div class="bg-white rounded-xl p-4 text-center shadow-sm">
                                            <div class="text-3xl font-bold text-green-600">${sessions.reduce((sum, s) => sum + (s.active_device_count || 0), 0)}</div>
                                            <div class="text-xs text-slate-500">Active Devices</div>
                                        </div>
                                        <div class="bg-white rounded-xl p-4 text-center shadow-sm">
                                            <div class="text-3xl font-bold text-slate-700">${sessions.reduce((sum, s) => sum + (s.total_scan_count || 0), 0)}</div>
                                            <div class="text-xs text-slate-500">Total Scans</div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-6">
                                        <h2 class="font-bold text-slate-800 mb-3 flex items-center gap-2">
                                            ...
                                            PSA Sessions (${psaSessions.length})
                                        </h2>
                                        <div class="space-y-3">
                                    ...
                                    <div class="mb-6">
                                        <h2 class="font-bold text-slate-800 mb-3 flex items-center gap-2">
                                            ...
                                            PML Sessions (${pmlSessions.length})
                                        </h2>
                                        <div class="space-y-3">
                                                    ...
                                                    >
                                                        <div class="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                ${isExpired ? '<div class="text-xs text-red-500 font-medium">⚠️ EXPIRED</div>' : ''}
                                                            </div>
                                                        </div>
                                        ...
                                        <div class="text-center">
                                            ...
                                            ${scan.isRawMaterial ? `
                                                ...
                                                <div class="mt-1 inline-block px-2 py-0.5 text-xs font-bold ${scan.unitType === 'kg' ? 'bg-teal-100 text-teal-700' : 'bg-blue-100 text-blue-700'} rounded-full">
                                                    ${scan.unitType === 'kg' ? '⚖️ Measured in KG' : '📦 Counted in Units'}
                                                </div>
                                            ` : `
                            ...
                            <div class="max-w-lg w-full bg-white rounded-3xl shadow-xl shadow-slate-200 overflow-hidden">
                                ...
                                <div class="p-4 sm:p-6 space-y-6 max-h-[75vh] overflow-y-auto">
                                    ...
                                    <div>
                                        <div class="flex items-center justify-between mb-3">
                                            <h3 class="font-bold text-slate-800 flex items-center gap-2">
                                                <span class="text-xl">🏷️</span> Floor Locations
                                            </h3>
                        ...
                        return `
                            ...
                            return `
                                            ...
                                            <div class="mt-4 text-center text-xs text-slate-400">
                                                ⬅️ Left to Right ➡️ &nbsp;|&nbsp; ⬇️ A (ground) to ${String.fromCharCode(64 + selectedRack.levels)} (top) ⬆️
                                            </div>
                                        </div>
                                    ` : `
